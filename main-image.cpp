...
void DCT(const float* A, float* C, int blockSize)
{
	...
	for (int u = 0; u <= 7; u++) {
		float Cu1 = 0;
		for (int C_u = 0; C_u <= 7; C_u++) {
			Cu1 += std::pow((std::cos(((2 * C_u + 1) * (u * M_PI)) / (2.0 * 8.0))), 2);
		}
		float Cu = 1.0 / std::sqrt(Cu1);

		for (int v = 0; v <= 7; v++) {
			float Cv1 = 0.0;
			for (int C_v = 0; C_v <= 7; C_v++) {
				Cv1 += std::pow((std::cos(((2 * C_v + 1) * (v * M_PI)) / (2.0 * 8.0))), 2);
			}
			float Cv = 1.0 / std::sqrt(Cv1);

			float xSum = 0.0;
			for (int x = 0; x <= 7; x++) {
				float ySum = 0.0;
				for (int y = 0; y <= 7; y++) {
					ySum += Cu * Cv * (std::cos(((2 * x + 1) * (u * M_PI)) / (2 * 8.0))) * (std::cos(((2 * y + 1) * (v * M_PI)) / (2 * 8.0))) * A[x * 8 + y];
				}
				xSum += ySum;
			}
			C[u * 8 + v] = xSum;
	...
	for (int x = 0; x <= 7; x++) {
		...
		for (int y = 0; y <= 7; y++) {
			...
			for (int u = 0; u <= 7; u++) {
				float Cu = 0;
				for (int C_u = 0; C_u <= 7; C_u++) {
					Cu += std::pow((std::cos(((2 * C_u + 1) * (u * M_PI)) / (2.0 * 8.0))), 2);
				}
				Cu = 1.0 / std::sqrt(Cu);
				float ySum = 0.0;
				for (int v = 0; v <= 7; v++) {
					float Cv = 0.0;
					for (int C_v = 0; C_v <= 7; C_v++) {
						Cv += std::pow((std::cos(((2 * C_v + 1) * (v * M_PI)) / (2.0 * 8.0))), 2);
					}
					Cv = 1.0 / std::sqrt(Cv);
					ySum += Cu * Cv * (std::cos(((2 * x + 1) * (u * M_PI)) / (2 * 8.0))) * (std::cos(((2 * y + 1) * (v * M_PI)) / (2 * 8.0))) * C[u * 8 + v];
				}
				xSum += ySum;
			}
			B[x * 8 + y] = xSum;
