...
void computeJointTransformations(
	...
	const std::vector<int>& p_jointParent, // p
	const unsigned int p_numJoints, // n
	std::vector<Matrix4f>& p_global) // p_local T, p_offset R, p_jointParent P(j) p_global A(rest) / F(animated)
{
	// TASK 1 comes here
...
void skinning(
		const std::vector<Vector3f>& p_vertices, // v
		const unsigned int p_numJoints, // n
		const std::vector<Matrix4f>& p_jointTrans, // F
		const std::vector<Matrix4f>& p_jointTransRestInv, // -A
		const std::vector<std::vector<float>>& p_weights, // W
